...
async function getStateParliamentData(slug: string) {
  ...
  return {
    ...
    years: elections?.map(e => ({
      year: e.year,
      title: e.title || `${e.year} General Election`,
...
export default async function StateParliamentPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const { state, years } = await getStateParliamentData(slug);
        ...
        <div className="premium-container relative z-10">
          <div className="max-w-4xl">
            ...
            <div className="flex items-center gap-4 mb-8">
               ...
               <h2 className="text-[10px] font-bold uppercase tracking-[0.4em] text-muted">National Mandate</h2>
            </div>
            
            <h1 className="font-serif text-8xl md:text-9xl font-bold mb-10 tracking-tighter leading-none">
              Lok Sabha <br/> <span className="italic text-muted-saffron">Parliament.</span>
            </h1>
            
            <p className="editorial-text text-3xl text-muted font-serif italic max-w-2xl leading-tight">
              &quot;Exploring how {state.name} voted to shape the destiny of the republic. The history of parliamentary mandates from the heartland.&quot;
            </p>
          </div>
        </div>
      </section>
           ...
           {years.length > 0 ? (
             <div className="space-y-24">
               <header className="flex justify-between items-end border-b border-border pb-12">
                  <div className="flex items-center gap-3">
                     ...
                     <h3 className="font-serif text-4xl font-bold tracking-tight italic">Archived Sessions</h3>
                  </div>
                  <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted">
                    Showing {years.length} verified general elections
                  </span>
               </header>
               <YearArchive stateName={state.name} stateSlug={slug} electionType="parliament" years={years as any} />
             </div>
           ) : (
             <div className="py-40 text-center flex flex-col items-center max-w-2xl mx-auto">
                <div className="w-24 h-24 rounded-full bg-card/40 border-2 border-border border-dashed flex items-center justify-center mb-10 opacity-30">
                   <Globe className="w-10 h-10 text-muted-saffron" />
                </div>
                <h2 className="font-serif text-5xl font-bold mb-8 tracking-tighter italic text-muted">Digitization in Progress.</h2>
                <p className="editorial-text text-xl text-muted/60 font-serif italic leading-relaxed">
                  The parliamentary history of {state.name} is currently being synchronized with the national archives.
                </p>
